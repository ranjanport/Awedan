...
async def render_LoginPage(request:Request):
     return templates.TemplateResponse('routes/login.html',  {"request" : request, "year" : datetime.date.today().year})

@authRouter.get('/singup', response_class=HTMLResponse)
async def render_SingupPage(request:Request):
    return templates.TemplateResponse("routes/singup.html", {"request" : request, "year" : datetime.date.today().year})

@authRouter.get("/signup-redirect/sign-in", response_class=HTMLResponse)
async def render_SingupPage_Redirect(request:Request):
    return templates.TemplateResponse("routes/sign-in-redirect.html", {"request" : request, "year" : datetime.date.today().year})
